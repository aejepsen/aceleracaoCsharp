...
{   
    ...
    public async Task TestPlataformWelcomeSuccess(string name, bool isCompany, CurrencyEnum currency)
    {
        ...
        var response = await ApiGetRequest("PlataformWelcome", token);
    ...
    {   
        var response = await ApiGetRequest("PlataformWelcome", invalidToken);

        response.StatusCode.Should().Be(HttpStatusCode.Unauthorized);
    }

    public async Task<HttpResponseMessage> ApiGetRequest(string route, string token)
    {
        var client = _factory.CreateClient();
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        return await client.GetAsync($"/Client/{route}");
    }
}
...
{   
    ...
    public async Task TestNewPromoAlertSuccess(string name, bool isCompany, CurrencyEnum currency)
    {
        ...
        var response = await ApiGetRequest("NewPromoAlert", token);
