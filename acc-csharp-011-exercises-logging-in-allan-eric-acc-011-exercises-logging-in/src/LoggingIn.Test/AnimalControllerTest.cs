...
public class AnimalControllerTest : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        var animalList = AutoFaker.Generate<Animal>(3);
        _repositoryMock.Setup(r => r.GetAll()).Returns(animalList);

        var res = await _client.GetAsync("/animals");
        var animalContent = await res.Content.ReadFromJsonWithEnumAsync<IEnumerable<Animal>>();

        res.StatusCode.Should().Be(HttpStatusCode.OK);
        //animalContent.Should().BeEquivalentTo(animalList);
    ...
    {
        var animalId = AutoFaker.Generate<Animal>();
        animalId.Id = 1;

        _repositoryMock.Setup(r => r.GetById(animalId.Id)).Returns(animalId);

        var res = await _client.GetAsync("/animals/1");
        var content = await res.Content.ReadFromJsonWithEnumAsync<Animal>();

        res.StatusCode.Should().Be(HttpStatusCode.OK);
        // content.Should().BeEquivalentTo(animalId);
    ...
    {
        var req = AutoFaker.Generate<AnimalRequest>();

        _repositoryMock.Setup(r => r.GetNextIdValue()).Returns(1);
        _repositoryMock.Setup(r => r.Create(It.Is<Animal>(r => r.Id == 1))).Returns(true);

        var res = await _client.PostAsJsonAsync("/animals", req);
        var content = await res.Content.ReadFromJsonWithEnumAsync<Animal>();

        res.StatusCode.Should().Be(HttpStatusCode.Created);

        content!.Id.Should().Be(1);
        content.Name.Should().Be(req.Name);
        content.Species.Should().Be(req.Species);
    ...
    public async Task UpdateSuccessTest()
    {
        ...
        var res = await _client.PutAsJsonAsync("/animals/1", req);
    ...
    public async Task DeleteSuccessTest()
    {
        ...
        var res = await _client.DeleteAsync("/animals/1");
    ...
    {
        var res = new List<HttpResponseMessage> {
